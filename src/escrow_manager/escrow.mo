...
actor class EscrowCanister(projectId: Types.ProjectId, recipient: Principal) = this {

    stable var nextSubAccount : Nat = 1_000_000_000;

    // CONSTS
    let FEE : Nat64 = 13_000;
    let FundNation_ACCOUNT = "8ac924e2eb6ad3d5c9fd6db905716aa04d949fe1a944442844214f59cf024e53";
    ...
    type ProjectState = {//projectstates 
        #approved: [Principal];//#whitelist: [Principal];
        #live;
        #closed;
        #noproject;
    };
    let Backend = actor "r7inp-6aaaa-aaaaa-aaabq-cai" : actor { //backend actor which will be used to get the current projectstate
    ...
    let Ledger = actor "rno2w-sqaaa-aaaaa-aaacq-cai" : actor { 
    ...
    type AccountIdAndTime = {      //accountid and data type
        accountId   : AccountIdText;
        time        : Time.Time;
    };
    //creating different variables used in the program 
    ...
    public func addConfirmedAccountsToConfirmedAccountsArray () : async () {
        ...
        for (kv in Trie.iter<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo)) {//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo)) {
    ...
    public func getNewAccountId (principal: Principal) : async Result.Result<AccountIdText, Text> {//tier: NFTInfoIndex) : async Result.Result<AccountIdText, Text> {
        ...
        switch (projectState) {
            case (#approved(approved)) {
                if (
                    Array.filter<Principal>(approved, isEqPrincipal).size() == 0
                    //and (await CNFT_NFT_Canister.principalOwnsOne(principal)) == false
                ) return #err("Principal is not on approved.");
            }; case (#closed) {
        ...
        accountInfo := Trie.putFresh<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(accountIdText), Text.equal, (principal, #empty, subaccountBlob));//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(accountIdText), Text.equal, (principal, #empty, subaccountBlob, tier));
    ...
    public func confirmTransfer(a : AccountIdText) : async Result.Result<(), Text> {
        switch(Trie.get<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(a), Text.equal)) {//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(a), Text.equal)) {
            case (?pssi) { 
                if (pssi.1 == #empty) {
                    accountInfo := Trie.replace<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(a), Text.equal, ?(pssi.0, #confirmed, pssi.2)).0;//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(a), Text.equal, ?(pssi.0, #confirmed, pssi.2, pssi.3)).0;
    ...
    public func cancelTransfer(a : AccountIdText) : async () {
        switch(Trie.get<AccountIdText,(Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(a), Text.equal)) {//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(a), Text.equal)) {
            case (?pssi) { 
                if (pssi.1 == #empty) {
                    accountInfo := Trie.replace<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(a), Text.equal, ?(pssi.0, #cancelled, pssi.2)).0;//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(a), Text.equal, ?(pssi.0, #cancelled, pssi.2, pssi.3)).0;
                } else {
    ...
    func cancelOpenAccountIds() : async () {
        ...
        for (acc in emptyAccounts.vals()) {
            ...
            let time = acc.time;
            if (time < cutoff) {
                switch(Trie.get<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(accountIdText), Text.equal)) {//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(accountIdText), Text.equal)) {
                    case (?pssi) { 
                        if (pssi.1 == #empty) {
                            accountInfo := Trie.replace<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo, accIdTextKey(accountIdText), Text.equal, ?(pssi.0, #cancelled, pssi.2)).0;//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo, accIdTextKey(accountIdText), Text.equal, ?(pssi.0, #cancelled, pssi.2, pssi.3)).0;
                        };
                    };
                    case null { };
                };
   ...
   /* type NFTStats = Types.NFTStats;
    ...
    public query func getAccountsInfo () : async Text { 
        ...
        var csv = "accountId,principal,subaccountStatus,subaccountBlob\n";//"accountId,principal,subaccountStatus,subaccountBlob,nftIndex\n";
        for (kv in Trie.iter<AccountIdText, (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo)) { //(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo)) {
            let accountIdText = kv.0;
            let principal = kv.1.0;
            let status = kv.1.1;
...
*/
    func principalHasUncancelledSubaccount (p : Principal) : Bool {
        for (ss in Trie.iter<AccountIdText,  (Principal, SubaccountStatus, SubaccountBlob)>(accountInfo)) {//(Principal, SubaccountStatus, SubaccountBlob, NFTInfoIndex)>(accountInfo)) {
